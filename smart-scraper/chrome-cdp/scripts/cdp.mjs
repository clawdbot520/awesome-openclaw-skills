...
async function getWsUrl() {
  ...
  // Try direct probe of common ports as fallback or if portFile fails
  const probePorts = [9222, 9223, 9224];
  const host = process.env.CDP_HOST || '127.0.0.1';

  if (!portFile) {
    // If no port file, try direct probe
    ...
    throw new Error('No DevToolsActivePort found and common ports (9222-9224) are not responding. Enable remote debugging.');
  }

  const lines = readFileSync(portFile, 'utf8').trim().split('\n');
  if (lines.length < 2 || !lines[0] || !lines[1]) {
    // Port file might be stale, try probe anyway
    for (const port of probePorts) {
      try {
        const res = await fetch(`http://${host}:${port}/json/version`, { signal: AbortSignal.timeout(500) });
        if (res.ok) {
